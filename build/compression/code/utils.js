...
const readFileSync = (filePath) => {
  try {
    ...
    log.error(`↓ "${filePath}": walk dir error`);
...
const createFileSync = (filePath, content) => {
  ...
  if (!fs.existsSync(dirname)) {
    try {
      ...
      log.error(`↓ "${dirname}": walk dir error`);
      log.error(err.message);
      process.exit();
    }
  }
  ...
  try {
    ...
    log.error(`↓ "${output}": walk dir error`);
    log.error(err.message);
    process.exit();
  }
}
...
function getDirFiles(dir) {
  ...
  // 判断路径是否存在
  if (!fs.existsSync(dir)) log.error(`↓ ${dir} 不存在`);
